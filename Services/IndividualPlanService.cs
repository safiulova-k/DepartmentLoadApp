    ...
    public class IndividualPlanService
    {
            ...
            model.Lecturers = plans
                .Select(plan => new IndividualPlanLecturerRowViewModel
                {
                    ...
                    AssignedHours = assignedHoursMap.TryGetValue(plan.Id, out var hours)
                        ? (int)Math.Round(hours, MidpointRounding.AwayFromZero)
                        : 0
                })
                .ToList();
            ...
            int lecturerAcademicYearPlanId)
        {
            ...
            var disciplineRows = await _context.WorkloadRows
                ...
                .Where(x =>
                    x.AcademicYear == academicYear &&
                    disciplinePlanRecordIds.Contains(x.AcademicPlanRecordId))
                .ToListAsync();
            ...
            var practiceRows = await _context.PracticeWorkloadRows
                .AsNoTracking()
                .Where(x =>
                    x.PlanYear == academicYear &&
                    practicePlanRecordIds.Contains(x.AcademicPlanRecordId))
                .ToListAsync();
            ...
            var giaRows = await _context.GiaWorkloadRows
                ...
                .Where(x =>
                    x.PlanYear == academicYear &&
                    giaPlanRecordIds.Contains(x.AcademicPlanRecordId))
                .ToListAsync();
            ...
            int? yearActualRow)
        {
            ...
            for (var i = 0; i < rows.Count; i++)
            {
                ...
                sheet.Cell(targetRow, 4).Value = item.StudentsCount == 0
                    ? string.Empty
                    : item.StudentsCount;
                ...
                SetHourValue(sheet, targetRow, 8, item.CourseProjectHours);
        ...
        private sealed class IndividualPlanRowData
        {
            ...
            public decimal PracticeHoursGuidance { get; set; }
