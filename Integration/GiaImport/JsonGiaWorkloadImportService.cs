...
using DepartmentLoadApp.Integration.GiaImport.Models;
...
namespace DepartmentLoadApp.Integration.GiaImport
{
    public class JsonGiaWorkloadImportService : IGiaWorkloadImportService
    {
        ...
        public async Task EnsureYearImportedAsync(string year)
        {
            ...
            var hasRows = await _context.GiaWorkloadRows
            ...
            foreach (var item in data.Rows.Where(x => ToAcademicYearString(x.PlanYear) == normalizedYear))
            {
                if (item.HasStateExam)
                {
                    rows.Add(new GiaWorkloadRow
                    {
                        PlanYear = ToAcademicYearString(item.PlanYear),
        ...
        {
            var filePath = Path.Combine(_environment.ContentRootPath, "ImportData", "gia-workload.json");
